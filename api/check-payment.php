...
if ($secretKey === '') {
  ...
  echo json_encode(['status' => 'failed', 'error' => 'Configure PAYEVO_SECRET_KEY (env) or PAYEVO_SECRET_FILE (path) or create /.payevo_secret_key (or /payevo_secret_key.txt)']);
  exit;
}
...
if ($status === 'paid') {
  ...
  $ctx['status'] = 'paid';
  if (!isset($ctx['notified']) || !is_array($ctx['notified'])) {
    $ctx['notified'] = [
      'pix_generated' => false,
      'pix_approved' => false
    ];
  }
  if (!isset($ctx['utmify_created_at']) || !is_string($ctx['utmify_created_at']) || trim($ctx['utmify_created_at']) === '') {
    $ctx['utmify_created_at'] = gmdate('Y-m-d H:i:s');
  }
  if (!isset($ctx['utmify_order_id']) || !is_string($ctx['utmify_order_id']) || trim($ctx['utmify_order_id']) === '') {
    $ctx['utmify_order_id'] = isset($ctx['payment_code']) ? (string)$ctx['payment_code'] : (string)$code;
  }
  $ctx['utmify_approved_at'] = gmdate('Y-m-d H:i:s');

  if (!isset($ctx['notified']['pix_approved']) || $ctx['notified']['pix_approved'] !== true) {
    $notifyApproved = notify_event('pix_approved', $ctx);
    if (is_array($notifyApproved) && isset($notifyApproved['sent']) && $notifyApproved['sent'] === true) {
      $ctx['notified']['pix_approved'] = true;
    }
    save_payment_context($code, $ctx);
  }
}
